...
UBXNAVRELPOSNED parse_ubx_nav_relposned(const std::vector<uint8_t>& payload) {
    ...
    if (payload.size() >= 40) { // UBX-NAV-RELPOSNED payload er 40 bytes
        try {
            ...
            uint32_t flags = *reinterpret_cast<const int32_t*>(&payload[36]);
...
void read_gps2(const std::string& port, DWORD baud_rate) {
    gps2_handle = CreateFileA(("\\\\.\\" + port).c_str(), GENERIC_READ, 0, NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL);
    ...
    if (!GetCommState(gps2_handle, &dcbSerialParams)) {
        std::cerr << "Failed to get COMM state for " << port << std::endl;
        CloseHandle(gps2_handle);
        gps2_handle = INVALID_HANDLE_VALUE;
        return;
    }
    ...
    if (!SetCommState(gps2_handle, &dcbSerialParams)) {
    ...
    if (!SetCommTimeouts(gps2_handle, &timeouts)) {
    ...
    DWORD bytesRead;
    while (running) {
        if (ReadFile(gps2_handle, data, sizeof(data), &bytesRead, NULL)) {
            if (bytesRead > 0) {
